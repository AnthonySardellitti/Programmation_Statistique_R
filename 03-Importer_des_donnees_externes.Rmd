...
Renseigner un répertoire actif permet notamment d'éviter de saisir le chemin complet d'un fichier de données tel que *C:/Document/dataset/mon_fichier.txt*

Pour modifier le répertoire courant, on utilise la fonction `setwd`tel que *setwd("C:/Users/anthony/Formations")*
...
```{r, eval=FALSE}
d <- read.csv(file = "./dataset/Titanic.csv")
```
...
```{r, eval = FALSE}
d <- read_excel(path  = "./dataset/Titanic.xlsx")
```
