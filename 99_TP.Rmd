...
On utilise le fichier compréssé `AmongUs.7z` où l'ensemble des fichiers décrivent les statistiques des parties jouées sur le jeu *Among us*. Il y a un fichier par joueur nommé *UserX.csv*.
...
a. Télécharger le fichier compréssé `AmongUs.7z` et le déziper. La fonction `list.files()` devrait vous aider à extraire l'ensemble des noms de fichiers présents dans un répertoire (voir exemple ci-dessous).
...
a. À partir de la colonne `Game.Completed.Date`, construire une colonne appelée `Date` avec la date de la partie au format *yyyy-mm-dd*. Vérifier que la colonne est bien en type *Date*.
...
k. Constuire **un graphique adapté** permettant de visualiser les variations du taux de succès des imposteurs selon les régions serveurs.
