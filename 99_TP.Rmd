...
b. Construire une nouvelle colonne appelée `qsec_cut` qui est un découpage de la variable `qsec_cut` en 3 groupes.
c. Construire une nouvelle colonne appelée `litres100km_group` pour distinguer deux groupes de voiture : celles qui consomment plus que la moyenne et celles qui consomment moins.
...
g. Triez le dataset  par ordre alphabétique et afficher le `nom` du pokemon en première position.
h. Triez le dataset  par `weight_kg` en ordre **décroissant**, et afficher le `nom`du pokemon en première position.
i. Triez le dataset  par `attack` en ordre **décroissant** puis par `speed` en ordre **croissant**, et afficher le `nom` des 10 premiers pokemons.
