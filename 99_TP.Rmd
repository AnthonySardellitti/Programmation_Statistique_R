...
a. Compiler l'ensemble des fichiers *UserX.csv* présents dans le fichier zip dans un seul et même data frame. Il est recommandé d'utiliser une boucle. 

b. Modifier votre boucle afin d'ajouter une colonne dans le data frame final qui renseigne le numéro d'utilisateur disponible dans le nom de chaque fichier csv.

c. Stocker le data frame final dans un objet appelé *AmongUs*.

d. Supprimer la colonne `Rank.Change` qui est inutile.

e. Construire une fonction appelé `manage_na()` qui prend en entrée un vecteur `x` correspondant aux données et un vecteur `string_to_na` correspondant à la liste des chaînes de caractères qu'on souhaite transformer en `NA`. La fonction retournera le vecteur modifié.

f. Construire une boucle qui parcourt toutes les colonnes du data frame *AmongUs* et qui remplace les champs avec des `-` et `N/A` par des valeurs manquantes `NA`. Utilisez la fonction `manage_na()` créée précédemment.
...
d. À partir de la colonne `Time.to.complete.all.tasks`, construire une colonne appelée `Complete.all.tasks.sec` correspondant au temps pour compléter toutes les tâches en secondes.
...
j. Constuire **un graphique adapté** permettant de visualiser les variations de taux de succès des imposteurs selon les régions serveurs.
