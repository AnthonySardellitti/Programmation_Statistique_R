...
<br> 
📢 Attention, pour le bon déroulement du TP, intégrer l'argument `stringsAsFactors = FALSE` dans votre fonction d'importation. Cet argument permet de ne pas typer par défaut les variables `character` en `factor` lors de l'importation.
...
c. On souhaite que le type de chaque colonne corresponde au tableau ci-dessous. Construire 3 boucles:

* une boucle qui convertit en type `numeric` les colonnes concernées par le tableau les unes après les autres.
* une boucle qui convertit en type `character` les colonnes concernées par le tableau les unes après les autres.
* une boucle qui convertit en type `factor` les colonnes concernées les unes après les autres.
